...
type ActionCreateAlertRuleItem = Extract<InboxWorkItem, { kind: "action_create_alert_rule" }>;
...
export function ActionCreateAlertRuleCard({
	item,
	onDecide,
}: {
	item: ActionCreateAlertRuleItem;
		...
		<div className={`rounded-lg border p-4 ${severityBorderClass(item.severity)}`}>
			<div className="flex items-center justify-between gap-2 mb-2">
				<div className="flex items-center gap-2">
					...
					<span className="text-sm font-medium">Alert rule request</span>
				</div>
				<span
