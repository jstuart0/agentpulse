...
curl -s -X POST "${AGENTPULSE_URL:-http://localhost:3000}/api/v1/hooks/status" \
  ...
  -d '{"session_id": "'"$CODEX_SESSION_ID"'", "status": "<researching|implementing|testing|debugging|reviewing|documenting|planning|waiting>", "task": "<current task>", "plan": ["done step", ">> current step", "pending step"]}'
```
